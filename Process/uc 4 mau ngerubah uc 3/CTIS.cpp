...
bool CTIS::findUsernamePatient(string username) {
    for(int i = 0; i < CTISPatients.size(); i++) {
...
Patient *CTIS::patientValidation(string username, string password, string name) {
    for(int i = 0; i < CTISPatients.size(); i++) {
        if (CTISPatients[i].getUsername() == username
        && CTISPatients[i].getPassword() == password
        && CTISPatients[i].getName() == name)
...
CovidTest CTIS::showALlCovidTest() {
    for(int i = 0; i < CTISTesters.size(); i++) {
        for (int j = 0; j < CTISTesters[i].getTesterCovidTestCollection().size(); j++) {
            cout << CTISTesters[i].getTesterCovidTestCollection()[j] << endl;
        }
    }
}

int *CTIS::getIndexPatient(string username, string password, string name) {
    for(int i = 0; i < CTISPatients.size(); i++) {
        if (CTISPatients[i].getUsername() == username
            && CTISPatients[i].getPassword() == password
            && CTISPatients[i].getName() == name)
