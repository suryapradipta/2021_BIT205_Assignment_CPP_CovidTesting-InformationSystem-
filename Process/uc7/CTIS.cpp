...
CovidTest *CTIS::findCovidTestTestKit(string testID) {
    for(int i = 0; i < CTISTestCentres.size(); i++) {
        for(int j = 0; j < CTISTestCentres[i].getTestKitCollection().size();j++) {
            for(int k = 0; k< CTISTestCentres[i].getTestKit()[j].getCovidTestCollection().size(); k++) {
                if(CTISTestCentres[i].getTestKit()[j].getCovidTest()[k].getTestId() == testID)
                    return &CTISTestCentres[i].getTestKit()[j].getCovidTest()[k];
            }
        }
    }
    return nullptr;
...
void CTIS::showALlCovidTestTestKit() {
    for(int i = 0; i < CTISTestCentres.size(); i++) {
        cout << "==================== "<< endl;
        cout << CTISTestCentres[i] << endl;
        for(int j = 0; j < CTISTestCentres[i].getTestKitCollection().size();j++) {
            cout <<  CTISTestCentres[i].getTestKit()[j] << endl;
            for(int k = 0; k< CTISTestCentres[i].getTestKit()[j].getCovidTestCollection().size(); k++) {
                cout << CTISTestCentres[i].getTestKit()[j].getCovidTest()[k] << endl;
            }
         }
    }
}
...
TestKit *CTIS::findTestKitByCentreOfficer(CentreOfficer &centreOfficer, string kitID) {
    ...
    for(int i = 0; i < centreOfficer.getTestCentreCollection().size(); i++) {
        cout << centreOfficer << endl;
        cout << centreOfficer.getTestCentre()[i] << endl;
