...
void recordNewTest() {
    ...
    if(centreCovidTest->getTestKitCollection().empty())
        ...
        testKitCovidTest = CTISSystem.findTestKit(kitID);
    }
    // cari centre officer
    CentreOfficer *centreOfficerTest = CTISSystem.findCentreOfficer(centreCovidTest->getCentreName());
